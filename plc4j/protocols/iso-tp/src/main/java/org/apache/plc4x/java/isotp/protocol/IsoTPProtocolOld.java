...
public class IsoTPProtocolOld extends PlcMessageToMessageCodec<IsoOnTcpMessage, IsoTPTpdu> {

    private static final Logger logger = LoggerFactory.getLogger(IsoTPProtocolOld.class);

    private short callingTsapId;
    private short calledTsapId;
    private TpduSize tpduSize;

    public IsoTPProtocolOld(short callingTsapId, short calledTsapId, TpduSize tpduSize) {
    ...
    @Override
    protected void encode(ChannelHandlerContext ctx, IsoTPTpdu in, List<Object> out) {
        ...
        switch (in.getTpduCode()) {
            ...
            case CONNECTION_CONFIRM:
                ...
                break;
            case DATA:
                encodeDataTpdu((IsoTPDataTpdu) in, buf);
                break;
    ...
    private void encodeErrorTpdu(IsoTPTpdu in, ByteBuf buf) {
        IsoTPErrorTpdu errorTpdu = (IsoTPErrorTpdu) in;
    ...
    private void encodeDisconnectTpdu(IsoTPTpdu in, ByteBuf buf) {
        IsoTPDisconnectTpdu disconnectTpdu = (IsoTPDisconnectTpdu) in;
        buf.writeShort(disconnectTpdu.getDestinationReference());
        buf.writeShort(disconnectTpdu.getSourceReference());
        if (disconnectTpdu instanceof IsoTPDisconnectRequestTpdu) {
            IsoTPDisconnectRequestTpdu disconnectRequestTpdu = (IsoTPDisconnectRequestTpdu) disconnectTpdu;
            buf.writeByte(disconnectRequestTpdu.getDisconnectReason().getCode());
        }
        encodeParameters(buf, in.getParameters());
    }

    private void encodeDataTpdu(IsoTPDataTpdu in, ByteBuf buf) {
    ...
    private void encodeConnectionTpdu(IsoTPTpdu in, ByteBuf buf) {
        IsoTPConnectionTpdu connectionTpdu = (IsoTPConnectionTpdu) in;
    ...
    protected void decode(ChannelHandlerContext ctx, IsoOnTcpMessage in, List<Object> out) {
        ...
        if (tpdu != null) {
            ...
            if (tpdu instanceof IsoTPConnectionConfirmTpdu) {
                tpdu.getParameter(TsapParameterCalled.class).ifPresent(
...
//            out.add(new IsoTPMessage(tpdu, userData));
    ...
    private IsoTPTpdu decodeErrorTpdu(ByteBuf userData, List<Parameter> parameters) {
        IsoTPTpdu tpdu;
        short destinationReference = userData.readShort();
        RejectCause rejectCause = RejectCause.valueOf(userData.readByte());
        tpdu = new IsoTPErrorTpdu(destinationReference, rejectCause, parameters, userData);
        return tpdu;
    }

    private IsoTPTpdu decodeDisconnectTpdu(ByteBuf userData, TpduCode tpduCode, List<Parameter> parameters) {
        IsoTPTpdu tpdu;
        short destinationReference = userData.readShort();
    ...
    private IsoTPTpdu decodeDataTpdu(ByteBuf userData, List<Parameter> parameters) {
        ...
        tpdu = new IsoTPDataTpdu(eot, tpduRef, parameters, userData);
        return tpdu;
    }

    private IsoTPTpdu decodeConnectTpdu(ChannelHandlerContext ctx, ByteBuf userData, TpduCode tpduCode, List<Parameter> parameters) {
        IsoTPTpdu tpdu;
        short destinationReference = userData.readShort();
        short sourceReference = userData.readShort();
        ...
        if (tpduCode == TpduCode.CONNECTION_REQUEST) {
            tpdu = new IsoTPConnectionRequestTpdu(destinationReference, sourceReference, protocolClass, parameters, userData);

        } else { // TpduCode.CONNECTION_CONFIRM
            tpdu = new IsoTPConnectionConfirmTpdu(destinationReference, sourceReference, protocolClass, parameters, userData);
     ...
     */
    private short getHeaderLength(IsoTPTpdu tpdu) {
        if (tpdu == null) {
            return 0;
        }
